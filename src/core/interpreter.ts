...
export default class SweetDB extends Sweet.Database {
  ...
  constructor (file: string) {
    ...
    this.content = FS.readFileSync(path.resolve(path.dirname(require.main.filename), file), 'utf-8')
      .split(/\r?\n/g)
            ...
            switch (token) {
              ...
              case 'TABLE': {
                const table_name = value
                  ...
                  .replace('{', '')
                  .trim()
                new Sweet.Table(`${current_database}:${table_name}`)
              ...
              case 'CLOJURE': {
                if (context.includes('FIELD')) {
                  ...
                  current_field += '}}'
                  new Sweet.Field(`${current_database}:${current_table}`, current_field_name, JSON.parse(current_field))
                  current_field = ''
                }
                break
              }
